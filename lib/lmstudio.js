...
Structure JSON attendue :
{
  ...
  "tags": ["tag1", "tag2", "tag3"],
  "dossier_suggere": "Nom du dossier de rangement, basé sur le DOMAINE THÉMATIQUE (comme le champ categorie), jamais sur la nature/format du contenu (article, patch notes, vidéo...). Dossiers déjà existants : __DOSSIERS__. Parmi ceux-ci, n'en réutilise un QUE s'il désigne lui-même un domaine thématique cohérent avec le sujet, même partiellement — ignore un dossier existant qui serait en réalité une nature de contenu (ex: 'Patch Notes', 'Vidéos') plutôt qu'un domaine. Si aucun dossier existant ne convient, propose un nouveau nom court (1 à 3 mots) désignant ce domaine (ex: pour un contenu Valorant, 'Esports', jamais 'Patch Notes').",
  "republication": {
...
export async function qualifySource(rawContent, dossiers) {
    // Les dossiers sont désormais générés par le modèle : un nom contenant
    // "$" corromprait le remplacement si on passait une string (patterns
    // spéciaux $&, $', $\` etc.). Une fonction de remplacement les ignore.
